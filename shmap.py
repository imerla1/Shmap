...
power_up_sound_dir = path.join(path.dirname(__file__), 'power_up_sounds')
explosion_list = listdir(explosion_dir)


def show_go_screen():
    draw_score(screen, "SHMUP!", 64, WIDTH / 2, HEIGHT / 4)
    draw_score(screen, "Arrow keys move, Space to fire", 22, WIDTH / 2, HEIGHT / 2)
    draw_score(screen, "Press a key to begin", 18, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(fps)
        for each_event in pygame.event.get():
            if each_event.type == pygame.QUIT:
                pygame.quit()
            if each_event.type == pygame.KEYUP:
                waiting = False
...
class Player(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        ...
        if self.power >= 2 and pygame.time.get_ticks() - self.power_timer > power_up_time:
            self.power -= 1
            self.power_timer = pygame.time.get_ticks()
...
# Game Sounds
shield_sound = pygame.mixer.Sound(path.join(power_up_sound_dir, 'shield.wav'))
gun_sound = pygame.mixer.Sound(path.join(power_up_sound_dir, 'gun.wav'))
